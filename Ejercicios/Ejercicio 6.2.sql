...
Ejercicio 1.- Obtener el nombre, cargo, salario de todos los empleados, siempre que el salario sea superior
a 800.000 y el cargo sea “Vendedor”, ordenado por el salario (de mayor a menor). Muestra el valor del
salario en dolares y en una columna llamada DOLARES.
*Se entenderá que 1€ es equivalente a 1.5$

SELECT NOMBRE,CARGO,SALARIO*1.5 AS DOLARES
FROM EMPLEADO
WHERE SALARIO > 800000 AND CARGO = "VENDEDOR"
ORDER BY SALARIO DESC

Ejercicio 2.- Obtener los nombres de los empleados cuya comisión sea menor o igual que el 30% de su
salario en una columna llamada “Privilegiados”. Ordena los resultados por el género de los empleados
(muestra primero a los de género "M"), y en su defecto por el salario del empleado (de mayor a menor).

SELECT NOMBRE AS PRIVILEGIADOS, GENERO
FROM EMPLEADO
WHERE COMISION <= (SALARIO*0.30)
ORDER BY GENERO DESC, SALARIO DESC;

Ejercicio 3.- Obtener el nombre, fecha de nacimiento y cargo de los empleados cuyo nombre tenga entre
10 y 15 caracteres (ambos no incluidos), o que en 2023 hayan cumplido 20 o más años desde su
incorporación a la empresa. Ordena los resultados por orden alfabético por el nombre de los empleados.

SELECT NOMBRE,FECHA_NAC, CARGO
FROM EMPLEADO
WHERE CHAR_LENGTH(NOMBRE) BETWEEN "11" AND "14" OR YEAR(FECHA_INCORPORACION <= 2003)
ORDER BY NOMBRE;

Ejercicio 4.- Obtener el nombre y salario de los 5 hombres con mayor salario del departamento
“MERCADEO”, siempre y cuando el nombre de la ciudad del departamento contenga alguna de las vocales
"i", "a" o "e". Ordena los resultados por el salario de los empleados (de mayor a menor).

SELECT E.NOMBRE,E.SALARIO
FROM DEPARTAMENTO AS D INNER JOIN EMPLEADO AS E ON D.COD=E.DEPARTAMENTO
WHERE E.GENERO LIKE M AND D.NOMBRE = "MERCADEO" AND (D.CIUDAD LIKE "%I%" OR D.CIUDAD LIKE "%A%" OR D.CIUDAD LIKE "%E%")
ORDER BY E.SALARIO DESC
LIMIT 5; 

Ejercicio 5.- Obtener el número total de mujeres de la empresa, junto con la media de salario de estas,
siempre que no trabajen en el departamento de VENTAS ni en el departamento de MANTENIMIENTO.
...
FROM DEPARTAMENTO AS D INNER JOIN EMPLEADO AS E ON D.COD=E.DEPARTAMENTO
WHERE D.NOMBRE NOT IN ("VENTAS","MANTENIMIENTO") AND E.GENERO = "F"